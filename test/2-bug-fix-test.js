...
describe("Bugs", function () {
  ...
  it("should understand TRUE", function () {
    ...
    XLSX_CALC(workbook);
    assert.strictEqual(workbook.Sheets.Sheet1.A1.v, 24);
  });
  ...
  it("should calc 6^4-4^4 = 1040", function () {
    ...
    XLSX_CALC(workbook);
    assert.strictEqual(workbook.Sheets.Sheet1.A2.v, 1040);
  });
  it("should consider the end of string", function () {
    workbook.Sheets.Sheet1.A1.f =
      'IF($C$3<=0,"Tempo de Investimento Invalido",IF($C$3<=24,"x","y"))';
    workbook.Sheets.Sheet1.C3 = { v: 24 };
    XLSX_CALC(workbook);
    assert.strictEqual(workbook.Sheets.Sheet1.A1.v, "x");
  });
  ...
  it("should works", function () {
    ...
    XLSX_CALC(workbook);
    assert.strictEqual(workbook.Sheets.Sheet1.A2.v, 1054);
  });
  ...
  it("some bug?", () => {
    let workbook = {
      Sheets: {
        Sample: {
          ...
          B1: {
            f: 'CONCATENATE("I"," told"," you ",SUM(1,2,3,4)," times",".","")',
          },
          B2: {
            f: 'CONCATENATE("I"," told"," you ",SUM(1,2,3,SUM(4))," times",".","")',
          },
          B3: {
            f: 'CONCATENATE("I"," told"," you ",SUM(1,2,3,SUM(4),0)," times",".","")',
          },
        },
      },
    };
    ...
    assert.strictEqual(sheet.A1.v, "I told you 10 times.");
    assert.strictEqual(sheet.A2.v, "I told you 10 times.");

    assert.strictEqual(sheet.B1.v, "I told you 10 times.");
    assert.strictEqual(sheet.B2.v, "I told you 10 times.");
    assert.strictEqual(sheet.B3.v, "I told you 10 times.");
  });
  ...
  it("fixes null is equal 0 in excel", () => {
    ...
    XLSX_CALC(workbook);
    let sheet = workbook.Sheets.Sample;
    assert.strictEqual(sheet.A2.v, "OK");
  });
  ...
  it("fixes Excel string comparison is case insensitive", () => {
    ...
    XLSX_CALC(workbook);
    let sheet = workbook.Sheets.Sample;
    assert.strictEqual(sheet.A1.v, "OK");
  });
  ...
  it("fixes reference%", () => {
    ...
    XLSX_CALC(workbook);
    assert.strictEqual(workbook.Sheets.Sheet1.A1.v, 0.2);
  });
  ...
  describe('"ref is an error with new formula" error thrown when executing a formula containing a number division by a blank cell', () => {
    it("should not run that division", () => {
      let workbook = {
        Sheets: {
          Sample: {
            A1: {},
            A2: {
              f: 'IF(AND(ISNUMBER(A1),A1<>0),100/A1,"Number cannot be divided by 0")',
            },
          },
        },
      };
      XLSX_CALC(workbook);
      let sheet = workbook.Sheets.Sample;
      assert.strictEqual(sheet.A2.v, "Number cannot be divided by 0");
    });
    it("should not run that multiplication", () => {
      let workbook = {
        Sheets: {
          Sample: {
            A1: {},
            A2: {
              f: 'IF(AND(ISNUMBER(A1),A1<>0),100*A1,"Number cannot be 0")',
            },
          },
        },
      };
      XLSX_CALC(workbook);
      let sheet = workbook.Sheets.Sample;
      assert.strictEqual(sheet.A2.v, "Number cannot be 0");
    });
  });
});
