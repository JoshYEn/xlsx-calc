...
describe("XLSX with XLSX_CALC", function () {
  function assert_values(sheet_expected, sheet_calculated) {
    for (var prop in sheet_expected) {
      if (prop.match(/[A-Z]+[0-9]+/)) {
        assert.strictEqual(
          sheet_calculated[prop].t,
          sheet_expected[prop].t,
          "Error: " + prop + ' f="' + sheet_expected[prop].f + '"'
        );
        if (sheet_calculated[prop].t === "e") return;
        if (
          typeof sheet_calculated[prop].v === "number" &&
          typeof sheet_expected[prop].v === "number"
        ) {
          assert.strictEqual(
            sheet_calculated[prop].v.toFixed(10),
            sheet_expected[prop].v.toFixed(10),
            "Error: " +
              prop +
              ' f="' +
              sheet_expected[prop].f +
              '"\nexpected ' +
              sheet_expected[prop].v +
              " got " +
              sheet_calculated[prop].v
          );
        } else {
          assert.strictEqual(
            sheet_calculated[prop].v,
        ...
        assert.strictEqual(
          sheet_calculated[prop].w,
          sheet_expected[prop].w,
          "Error: " + prop + ' f="' + sheet_expected[prop].f + '"'
        );
      }
    }
  ...
  it("erase_values_that_contains_formula sets values to null", function () {
    ...
    assert.strictEqual(workbook.Sheets.Sheet1.A1.v, null);
  });
  ...
  it("handles transpose", function () {
    ...
    sheet1.G13.v = null;
    XLSX_CALC(workbook);
    assert.strictEqual(sheet1.F13.v, 1);
    assert.strictEqual(sheet1.G13.v, 4);
    assert.strictEqual(sheet1.H13.v, 7);

    assert.strictEqual(sheet1.F14.v, 2);
    assert.strictEqual(sheet1.G14.v, 5);
    assert.strictEqual(sheet1.H14.v, 8);

    assert.strictEqual(sheet1.F15.v, 3);
    assert.strictEqual(sheet1.G15.v, 6);
    assert.strictEqual(sheet1.H15.v, 9);
  });
